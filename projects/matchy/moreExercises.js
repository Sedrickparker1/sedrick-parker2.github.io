...
function getNamesInOrder(coll) {
  for (let key in coll) {
    for (let i = 0; i < coll[key].length; i++) {
      let firstL = coll[key][i]['animal'].split('');
      // capitalziing all of the first letters of the names..
      
      console.log('Name : ' + firstL[0].toUpperCase() + coll[key][i]['animal'].slice(1));
    }
  }
